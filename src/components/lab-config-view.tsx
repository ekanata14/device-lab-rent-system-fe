...
import { AlertCircle, Clock, ShieldAlert, Plus, Trash2, Printer as PrinterIcon } from 'lucide-react';
...
export function LabConfigView({ settings, printers, onUpdate, onAddPrinter, onRemovePrinter }: LabConfigViewProps) {
    ...
    <div className="grid gap-8 max-w-4xl mx-auto pb-20">
      ...
      <Card className="border-destructive/30 bg-destructive/5 shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            Force the lab status to "Closed" immediately, overriding all schedules.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="flex items-center justify-between p-4 bg-background/50 rounded-lg border">
            <div className="space-y-0.5">
              ...
              <p className="text-sm text-muted-foreground">Locks all printer interactions for students.</p>
            </div>
            <Switch 
          ...
          {settings.isManuallyClosed && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                Students can currently view status but cannot initiate any reservations or queue requests.
              </AlertDescription>
            </Alert>
          )}
        </CardContent>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          <form onSubmit={handleAdd} className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-muted/30 rounded-lg border border-dashed">
            ...
            <div className="flex items-end">
              <Button type="submit" className="w-full bg-primary" disabled={!newPrinter.name || !newPrinter.model}>
          ...
          <div className="space-y-3">
            <h3 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground px-1">Active Fleet ({printers.length})</h3>
            <div className="grid gap-2">
              {printers.map((p) => (
                <div key={p.id} className="flex items-center justify-between p-3 bg-card border rounded-lg hover:border-primary/30 transition-colors">
                  <div>
                    <span className="font-bold mr-2">{p.name}</span>
                    <span className="text-xs text-muted-foreground">{p.model}</span>
                  </div>
                  <Button variant="ghost" size="sm" onClick={() => onRemovePrinter(p.id)} className="text-destructive hover:bg-destructive/10">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Set the operational window for automated lab access.
          </CardDescription>
        </CardHeader>
